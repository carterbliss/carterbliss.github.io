...
<div class="code-description">
  ...
  <strong> R3 (6k ohm)</strong> is dedicated as a shunt and sense resistor which is calculated by the voltage dropoff. V = I * R. 60 mV = 10 micra A * Rsense. Rsense = 6k ohm. By dropping the voltage across pins IN+ and IN-, the chip reads the difference in voltage and multiplies it by a gain and adds it to the reference. In our schematic, our reference is generated by an op-amp buffer that centers around a 1.65V output. We use an op-amp buffer here because the INA190 is bidirectional, meaning we will read both positive and negative current values. Positive current allows pulls analog values above 1.65V and negative current pulls analog values below 1.65V. Without this reference, negative current would read 0V and won't provide accurate experimental data. 
  <br><br>

  The equation for analog value output into pin <strong> IN1 </strong> is Vout = Vref + (Gain × Vsense)
     = 1.65 + (25 × 60mV)
     = 1.65 + 1.5V = 3.15V  (at +10µA)
     = 1.65 - 1.5V = 0.15V  (at -10µA). 
    We chose INA190 A1 for a gain of 25. 
Additionally on our output, we implemented a low-pass filter to filter any fluctuation in the analog read values. However, in the case a LPF is unnecessary, we placed a Do Not Populate symbol, representing that we intentionally won't solder on the filter unless we deem it necessary after experiment trials. Additionally, our output ranges to 3.15V, however the STM32 only intakes up to 3V, so we step down our voltage with a voltage divider by V_IN1 = Vout_INA × (10k / 11k) = Vout_INA × 0.909.
  <br><br>
  <strong>Voltage monitoring:</strong> For our monitoring voltage schematic, we chose a precision voltage reference/buffer <strong> OPA333 </strong> that reads the supercapacitor voltage and presents it as a stable, low-impedance signal to the ADC. The op-amp compares Voltage atop the supercapacitor with its output <strong> IN0 </strong>, essentially outputting the analog value of voltage readings. Similar to the monitoring current, we implemented another DNP low pass filter if we discover fluctuating values. Additionally, we step down the voltage to 3V to ensure we do not overload the STM32. 
  <br><br>
