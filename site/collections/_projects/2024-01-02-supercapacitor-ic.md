...
The transient simulation confirms the discharge circuit behaves as designed. The most notable result is that <strong>I(R1) holds at 9.99µA</strong> — effectively 10µA — throughout the discharge phase, validating the op-amp feedback loop's ability to maintain constant current. The supercapacitor voltage (purple trace) decreases linearly during each discharge interval, which is the expected signature of a constant-current load and what enables accurate capacitance calculation. The green trace shows the control switching signal cycling the circuit between charge and discharge states.
...
Further results will be added following the NASA balloon flight in summer 2026.
